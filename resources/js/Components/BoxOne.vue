...
const getProduct = async(page) => {
    const response = await sendRequest({
        method:'get',
        url:`/get-box-one-products?page=${page}`,
        params: query.value,
    })
...
<template>
    <section class="home-box home-box--one mb-8">
        <div class="container">
                        ...
                        <ul class="box-one">
                            <li>
                                <input
                                    ...
                                    checked
                                    hidden>
                                <label for="box-1-all" class="shadow" >All</label>
                            </li>
                            <li v-for="brand in brands" :key="brand.id">
                                <input
                                    type="radio"
                                    :id="`box-1-brand-${brand.id}`"
                                    :value="brand.id"
                                    v-model="query.brand_id"
                                    name="brand"
                                    hidden>
                                <label :for="`box-1-brand-${brand.id}`">{{ brand.name }}</label>
                            </li>
                        </ul>
                    </div>
                </div>
                <div class="home-box__inner__main-content home-box__inner__main-content--box-one">
                    <div class="home-box__inner__main-content__side-bar" :class="{'home-box__inner__main-content__side-bar--active' : isActive}">
                        <ul class="box-one-cats">
                            ...
                            >
                                ...
                                <input
                                    type="radio"
                                    :id="`box-1-category-${category.id}`"
                                    :value="category.id"
                                    v-model="query.category_id"
                                    name="category"
                                    hidden>
                                <label :for="`box-1-category-${category.id}`">
                    ...
                    <div class="home-box__inner__main-content__products p-3">
                        ...
                        <div v-if="products?.data?.length !== 0" class="row row-cols-2 row-cols-lg-4">
                            <ProductCard class="px-1 mb-3"  v-for="product in products?.data" :info="product" :key="product.id"/>
                        </div>
                        <div v-else>
