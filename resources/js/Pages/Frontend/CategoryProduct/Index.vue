                    ...
                    <div class="links-wrapper" :class="{'links-wrapper--toggled' : isActive}">
                        <ul class="bg-white shadow h-100">
                            <li v-for="category in  categories">
                                <Link :href="`/category/${category?.slug}`" class="links-item border-bottom d-flex align-items-center gap-1 fw-bold">
                                    <img :src="category?.image" :alt="category.name" style="width: 20px;height: 20px">
                                    {{category.name}}
                                </Link>
                                <ul>
                                    <li v-for="item in category?.sub_categories">
                                        <Link  :href="`/category/${category?.slug}/${item?.slug}`" class="links-item border-bottom d-flex align-items-center gap-1 ps-4">
                                          <img :src="item?.image" style="width:25px;height:25px;" :alt="item.name">
                                            {{item?.name}}
                                        </Link>
                                    </li>
                                </ul>
